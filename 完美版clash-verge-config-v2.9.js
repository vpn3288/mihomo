...
// Clash Verge Rev subscription script v2.9 (多前置链式代理安全增强版)
// 分流策略：
...
var FRONT_PROXY_GROUPS = [
  ...
  { name: "FrontProxyUS", filter: "美国|美國|US|United States|America", exclude: FRONT_NODE_EXCLUDE_FILTER, mode: "url-test" },
  { name: "FrontProxySG", filter: "新加坡|Singapore|SG|狮城|獅城", exclude: FRONT_NODE_EXCLUDE_FILTER, mode: "url-test" },
  { name: "FrontProxyJP", filter: "日本|Japan|JP|Tokyo|Osaka", exclude: FRONT_NODE_EXCLUDE_FILTER, mode: "url-test" }
];

// 每个落地订阅使用哪个前置组。数组会创建浏览器专属 select 组，方便在 Clash Verge 界面手动切换。
// 数组第一个值是默认优先项；例如 Edge 默认美国，也可手动切到日本/新加坡/全部前置。
var FRONT_PROXY_BY_PROVIDER = {
  Edge: ["FrontProxyUS", "FrontProxyJP", "FrontProxySG", "FrontProxy"],
  Chrome: ["FrontProxySG", "FrontProxyJP", "FrontProxyUS", "FrontProxy"],
  Firefox: ["FrontProxyJP", "FrontProxySG", "FrontProxyUS", "FrontProxy"],
  Brave: ["FrontProxy", "FrontProxySG", "FrontProxyJP", "FrontProxyUS"],
  LibreWolf: ["FrontProxy", "FrontProxySG", "FrontProxyJP", "FrontProxyUS"],
  Vivaldi: ["FrontProxy", "FrontProxySG", "FrontProxyJP", "FrontProxyUS"],
  Opera: ["FrontProxy", "FrontProxySG", "FrontProxyJP", "FrontProxyUS"],
  Default: ["FrontProxy", "FrontProxySG", "FrontProxyJP", "FrontProxyUS"],
  OtherApps: ["FrontProxy", "FrontProxySG", "FrontProxyJP", "FrontProxyUS"]
};
...
function validateFrontProxyMappings() {
  ...
  for (i = 0; i < keys.length; i++) {
    ...
    var options = getFrontProxyOptionsForKey(key);
    ...
    if (options) {
      ...
      validateFrontProxyOptions(key, options, names);
...
function getFrontProxyOptionsForKey(key) {
  var value = FRONT_PROXY_BY_PROVIDER && FRONT_PROXY_BY_PROVIDER[key];
  if (isArray(value)) {
    return value;
  }
