...
// Clash Verge Rev subscription script v2.10 (多前置链式代理安全增强版)
// 分流策略：
...
var FRONT_NODE_EXCLUDE_FILTER = "中转|中轉|Relay|Transit|Via|剩余|流量|套餐|官网|到期|过期|Expire|Expired|Traffic|Official|Website";
...
var FRONT_PROXY_BY_PROVIDER = {
  Edge: FRONT_REGION_PRESETS.us_first,
  Chrome: FRONT_REGION_PRESETS.asia_first,
  Firefox: FRONT_REGION_PRESETS.jp_first,
  Brave: FRONT_REGION_PRESETS.all_regions,
  LibreWolf: FRONT_REGION_PRESETS.all_regions,
  Vivaldi: FRONT_REGION_PRESETS.asia_first,
  Opera: FRONT_REGION_PRESETS.asia_first,
  Default: FRONT_REGION_PRESETS.all_regions,
  OtherApps: FRONT_REGION_PRESETS.all_regions
};
...
function pushFrontSelectorGroups(groups) {
  ...
  for (i = 0; i < keys.length; i++) {
    ...
    seen[name] = true;
    groups.push(makeFrontAutoFallbackGroup(key, options));
    groups.push({
      name: name,
      type: "select",
...
function getFrontSelectorOptionsForKey(key) {
  ...
  var fallbackName = getFrontAutoFallbackNameForKey(key);
