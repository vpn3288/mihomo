...
// Clash Verge Rev subscription script v2.13 (分浏览器前置地区链式代理生产增强版)
// 分流策略：
...
//
// 快速开始：
// 1. SUB.Front 填购买的前置订阅，SUB.Edge/Chrome/Firefox/Default 等填自建 VPS 落地订阅。
// 2. 前置地区来自同一个 SUB.Front，例如 FrontProxyUS/SG/JP/HK；地区组默认 url-test 自动选最快节点。
// 3. 浏览器规则仍走 EdgeProxy/ChromeProxy/FirefoxProxy，前置组只作为落地节点的 dialer-proxy。
// 4. 在 Clash Verge 里切换 EdgeFrontProxy/ChromeFrontProxy/FirefoxFrontProxy，即可给不同浏览器换前置地区。
// 5. IP 检测网站应该显示落地 VPS IP；切换前置地区只改变“本机到 VPS 的路径”，不会改变最终出口 IP。
// 6. 如果目标是任何网站都不能看到本机 IP，把 ALLOW_DIRECT_RULES 改为 false。
...
// 如果你的购买订阅把“中转/专线/IPLC”作为可用前置节点出售，可以从这里删除相关关键词。
var FRONT_NODE_EXCLUDE_FILTER = "中转|中轉|Relay|Transit|Via|IPLC|IEPL|专线|專線|→|➜|>>|剩余|剩餘|流量|套餐|官网|官網|到期|过期|過期|Expire|Expired|Traffic|Remain|Left|Official|Website|负载|負載|轮询|輪詢|故障|转移|轉移|自动|自動|Balance|Round|Robin|Failover|Auto";
...
var HEALTH_CHECK_URLS = {
  ...
  // 如果替换 front/landing URL，记得同步修改 HEALTH_CHECK_EXPECTED_STATUS。
