...
// Clash Verge Rev subscription script v2.7 (链式代理安全增强版)
// 分流策略：
...
// 前置节点选择策略：
// "fallback" 稳定优先，当前置挂掉才切换；"url-test" 延迟优先。
...
function validateFrontStrategy() {
  if (FRONT_SELECTION_MODE !== "fallback" && FRONT_SELECTION_MODE !== "url-test") {
    throw new Error("FRONT_SELECTION_MODE must be fallback or url-test");
  }
...
function buildGroups() {
  ...
  if (needsFrontProxy()) {
    groups.push(makeUrlTestGroup("FrontAuto", "provider-front", "front"));
...
function makeFrontProxyGroup() {
  ...
  return {
    name: "FrontProxy",
    type: FRONT_SELECTION_MODE === "url-test" ? "select" : "fallback",
    proxies: proxies,
    use: ["provider-front"],
    url: "https://www.gstatic.com/generate_204",
    interval: 60,
